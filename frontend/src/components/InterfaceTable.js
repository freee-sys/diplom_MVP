...
function InterfaceTable({ element, refreshElements, openAddInterface = false, onCloseAddForm, authConfig, canWrite = true }) {
  ...
  React.useEffect(() => {
    if (openAddInterface && canWrite) {
      setShowForm(true);
    }
  }, [openAddInterface, canWrite]);
  ...
  const handleAddInterface = async (e) => {
    ...
    try {
      ...
      setError('Failed to add interface: ' + (err.response?.data?.error || err.message));
    }
  };
  ...
  const handleUpdateInterface = async (e) => {
    ...
    try {
      ...
      setError('Failed to update interface: ' + (err.response?.data?.error || err.message));
    }
  };
  ...
  const handleDeleteInterface = async (interfaceId) => {
    ...
    if (window.confirm('Delete this interface?')) {
      try {
        ...
        setError('Failed to delete interface: ' + (err.response?.data?.error || err.message));
      }
    }
  };
